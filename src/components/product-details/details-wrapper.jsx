...
const DetailsWrapper = ({ productItem, handleImageActive, activeImg, detailsBottom = false, isModal = false }) => {
  const { sku, img, title, imageURLs, category, description, discount, price, discountedPrice, status, reviews, tags, offerDate } = productItem || {};
  ...
  const hasDiscount =
    (discountedPrice && discountedPrice < price) || (discount > 0);
  ...
  return (
    <div className={`tp-product-details-wrapper${isModal ? " es-modal-details" : ""}`}>
      ...
      <div className="tp-product-details-inventory d-flex align-items-center mb-10">
        <div className="tp-product-details-stock mb-10">
          <span className={status === "in-stock" ? "is-in-stock" : ""}>{stockLabel}</span>
        </div>
        ...
        {hasDiscount ? (
          <>
            ...
            <span className="tp-product-details-price new-price">
              ₹{Number(displayPrice).toLocaleString("en-IN", { maximumFractionDigits: 2 })}
            </span>
          </>
        ) : (
          <span className="tp-product-details-price new-price">
            ₹{Number(sellingPrice).toLocaleString("en-IN", { maximumFractionDigits: 2 })}
          </span>
        )}
      </div>
